...
data class CategorySpec(
    val id: String,
    val title: String,
    val source: String // res/raw/{source}.json (확장자 없이)
)
...
@Composable
private fun CategoriesListScreen(

    categories: List<CategorySpec>,
    onOpen: (CategorySpec) -> Unit
) {
    Scaffold(
        topBar = { CenterAlignedTopAppBar(title = { Text("단어 모음") }) }
    ) { inner ->
...
/* ------------------ 유틸: categories.json 로드 + 존재하는 raw만 필터 ------------------ */
private fun availableCategories(ctx: Context): List<CategorySpec> {
